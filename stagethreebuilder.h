...
public:
    ~StageThreeBuilder() {}
    /**
     * @brief StageThreeBuilder - make stage 3 game independent
     */
...
private:
    ...
    static bool isValidChannel(double xpos, double ypos, double radius, double tableWidth, double tableHeight);
    ...
    /**
     * @brief convertAndCheckChannel-  convert the channel to a valid one if possible (add defaults), or return an empty json if irrecoverably invalid
     * @param in
     * @param tableWidth
     * @param tableHeight
     * @return
     */
    static QJsonObject convertAndCheckChannel(QJsonObject in, double tableWidth, double tableHeight);
};
