...
bool StageThreeBuilder::isValidChannel(double xpos, double ypos, double radius, double tableWidth, double tableHeight) {
    if (xpos == DOUBLEINF || ypos == DOUBLEINF || radius == DOUBLEINF) return false;
    bool isWithinXBounds = xpos > -radius && xpos < tableWidth + radius;
    bool isWithinYBounds = ypos > -radius && ypos < tableHeight + radius;
    return isWithinXBounds && isWithinYBounds;
...
void StageThreeBuilder::addTable(QJsonObject &tableDatax) {
    ...
    if (tableDatax.contains("pockets")) {
        for (const QJsonValueRef v : cookedTableData.value("pockets").toArray()) {
            ...
            QJsonObject channelData = convertAndCheckPocket(v.toObject(), t->getWidth(), t->getHeight());
            // skip invalid pockie
            if (channelData.isEmpty()) continue;

            t->addPocket(m_factory->makePocket(channelData));
        }
    }

    if (tableDatax.contains("channels")) {
        for (const QJsonValueRef v : cookedTableData.value("channels").toArray()) {
            // clean and check pockie
            QJsonObject channelData = convertAndCheckPocket(v.toObject(), t->getWidth(), t->getHeight());
            // skip invalid pockie
            if (channelData.isEmpty()) continue;

            t->addChannel(m_factory->makeChannel(channelData));
        }
    }
...
QJsonObject StageThreeBuilder::convertAndCheckChannel(QJsonObject in, double tableWidth, double tableHeight) {
    // default radius = 15
    if (!in.contains("radius")) in["radius"] = 20.0;
    double radius = in.value("radius").toDouble(DOUBLEINF);
    // missing key
    if (!in.contains("position")) return QJsonObject();
